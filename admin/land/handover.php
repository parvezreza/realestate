...
<body>
	<div class="header">
    <!-- 	Log out					-->
    	<div class="top-bar"><h3>Land Project - Handed Over</h3><a href="../index.php?option=logout"><span>Logout</span><img src="../styles/icon-logout.gif"></a></div>
    	<!-- 																		-->
    ...
    <!--	Sub Navigation				-->            
        <div class="sub-tab">
        	<p>
            	<a href="handover.php"><img src="../styles/landproject-small.png" height="25"><span>All Handover</span></a>
            	<a href="handover.php?option=add"><img src="../styles/article-addnew.png" height="25"><span>Add New</span></a>
    ...
    <?php
		if($_POST)
		{
			if($_POST['editid'])
			{
				...
				$hDB->Query( "SELECT land_pro_over_name FROM tbl_land_project_handover WHERE land_pro_over_id = '$id'" );
				$obj = $hDB->Fetch();
				 
				if( !@rename( $basedir.$obj->land_pro_over_name, $basedir.$project ) )
				{
				...
				else
				{
					...
					$hDB->Query( "UPDATE tbl_land_project_handover SET land_pro_over_name = '$project' WHERE land_pro_over_id = '$id'" );
			...
			elseif($_POST['project_detail'])
			{
				...
				$hDB->Query( "UPDATE tbl_land_project_handover SET land_pro_over_feature = '$feature', land_pro_over_terms = '$terms' WHERE land_pro_over_name = '$project' " );
			...
			elseif($_POST['ckbox'])
			{
				foreach( $_POST["ckbox"] as $id )
				{
					...
					if( !Folder::Remove( $basedir.$obj->land_pro_over_name ) ) {
						Notify::Error( 'Permission denaied!<b>'.$obj->land_pro_over_name.'</b> project couldn\'t deleted.' );
					}
					else {
						$hDB->Query( "DELETE FROM tbl_land_project_handover WHERE land_pro_over_id = '$id'" );
						$nd++;
					}
				}
			...
			else
			{
				...
				else
				{
					...
					$hDB->Query( "INSERT INTO tbl_land_project_handover( land_pro_over_id, land_pro_over_name, land_pro_over_feature, land_pro_over_terms ) VALUES ( '', '$project', '', '' )" );
...
<!--   *****************		Article Add Form Module			***************** -->
	...
	?>  
        	...
        	<div class="module-header"><span>Create New Land Project</span></div>
            <div class="module-body">
            	<form action="handover.php?option=details" method="post" enctype="multipart/form-data" id="DataForm">
	...
	<?php
		...
		{
			$id = $_GET["projectid"];
			$hDB->Query( "SELECT * FROM tbl_land_project_handover WHERE land_pro_over_id = '$id'" );
	...
	?>
        ...
        <div class="module-60" style="float:left">
        	<div class="module-header"><span>Edit existing Land Project</span></div>
            <div class="module-body">
            	<form action="handover.php?option=details" method="post" enctype="multipart/form-data" id="DataForm">
                	<h3>Project Editing</h3>
                    <!--</span>-->
                    <p>Project Name</p>
                    	<input type="hidden" name="editid" value="<?php print $obj->land_pro_over_id ?>" />
                    	<input class="input medium" name="name" type="text" value="<?php print $obj->land_pro_over_name ?>" fv="char 5" ><span></span>
                        <p><i><b>Caution:</b> Use only Alphabet & Numeric character, don't use any special character!</i></p>
                    ...
                    &nbsp; <input type="text" class="input short-medium" name="thumbnail" value="<?php if(file_exists($basedir.$obj->land_pro_over_name.'/thumbnail.jpg')) print 'thumbnail.jpg' ?>" />
     ...
     <?php
		...
		elseif( $_GET["option"] == "details" )
		{
			if( $_GET["projectid"] || $_POST["editid"] ) {
				...
				$hDB->Query( "SELECT * FROM tbl_land_project_handover WHERE land_pro_over_id = '$id'" );
				$obj = $hDB->Fetch();
				$project = $obj->land_pro_over_name;
			}
			else {
				$project = trim( $_POST["name"] );
			}
    ...
    <!--		Project Details module			-->
   		<div class="module-95" style="float:left">
            ...
            <div class="module-body">
            <form action="handover.php" method="post" enctype="multipart/form-data" >
            	...
            	<div id="accordion">
                    ...
                    <div>
                        <textarea class="input long" rows="8" id="editor1" name="feature"><?php print $obj->land_pro_over_feature ?></textarea> 
                    </div>
                    <h3><a href="#">Terms & Condition</a></h3>
                    <div>
                       <textarea class="input long" rows="8" id="editor2" name="terms"><?php print $obj->land_pro_over_terms ?></textarea>
                    </div>
                    ...
                    <div>
                        ...
                        &nbsp;<input type="text" class="input short" name="application" value="<?php if(file_exists($basedir.$obj->land_pro_over_name.'/application.pdf')) print 'application.pdf' ?>" />
                    ...
                    <div>
                        ...
                        &nbsp;<input type="text" class="input short" name="pricelist" value="<?php if(file_exists($basedir.$obj->land_pro_over_name.'/pricelist.jpg')) print 'pricelist.jpg' ?>" />
                    ...
                    <div>
                        ...
                        &nbsp;<input type="text" class="input short" name="location" value="<?php if(file_exists($basedir.$obj->land_pro_over_name.'/location.jpg')) print 'location.jpg' ?>" />
                        <p><i><b>Hint:</b> Support only <b>jpg, png</b> & <b>gif</b> image format and max upload size is <b>300 KB</b>. 
                        	Already have one and don't want to change previous price list picture, leave this blank!</i></p>
 ...
 /*<!--	********************* 		noticeegory Table Module		******************* -->*/
	...
	?>
        ...
        <div class="module-65" style="float:left">
                <div class="module-header"><span>Land Projects - Handed Over</span></div>
                <div class="module-body-table">
                <form id="DataForm" action="handover.php" method="post">
                    <table border="0">
                        <thead>
                            <tr>
                        ...
                        <?php
                            ...
                            $hDB->Query( "SELECT * FROM tbl_land_project_handover ORDER BY land_pro_over_id ASC" );
                            ...
                            while( $obj = $hDB->Fetch() )
                            {
								...
								print "								
								<tr>
									<td align='center'><input type='checkbox' name='ckbox[]' id='ckbox' value='$obj->land_pro_over_id' /></td>
									<td align='center'>".$sn."</td>
									<td>".$obj->land_pro_over_name." 
										<a href='handover.php?option=edit&projectid=$obj->land_pro_over_id' title='Edit it!'>Edit</a>
									</td>
									<td align='center'>".$obj->land_pro_over_id."</td>
									<td style='border:none'>
										<a href='handover.php?option=details&projectid=$obj->land_pro_over_id'>Edit Details</a>
									</td>
								</tr>";
							}
						?>
